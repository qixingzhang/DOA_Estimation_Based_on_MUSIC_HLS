...
#ifndef _fft_streaming_Stavdy_H_
#define _fft_streaming_Stavdy_H_

#include <systemc>
#include <iostream>
#include <fstream>

#include "fft_streaming_Stavdy_memcore.h"

SC_MODULE(fft_streaming_Stavdy) {
...
protected:
    ...
    // buffer instances
    fft_streaming_Stavdy_memcore* buffer[BufferCount];
...
public:
    ...
    SC_CTOR(fft_streaming_Stavdy) : m_trace_file(0) {
        for (unsigned i = 0; i < BufferCount; i++) {
            char szTmp[128];
            sprintf(szTmp, "%d", i);
            std::string name = "fft_streaming_Stavdy_memcore_U_";
            name += szTmp;

            buffer[i] = new fft_streaming_Stavdy_memcore(name.c_str());
            buffer[i]->clk      ( clk );
        ...
        if (dump_vcd) {
            m_trace_file = sc_create_vcd_trace_file( "fft_streaming_Stavdy.trace");
            // ports
