...
#ifndef _qrf_top_Qi_M_real_H_
#define _qrf_top_Qi_M_real_H_

#include <systemc>
#include <iostream>
#include <fstream>

#include "qrf_top_Qi_M_real_memcore.h"

SC_MODULE(qrf_top_Qi_M_real) {
...
protected:
    ...
    // buffer instances
    qrf_top_Qi_M_real_memcore* buffer[BufferCount];
...
public:
    ~qrf_top_Qi_M_real();
    SC_CTOR(qrf_top_Qi_M_real) : m_trace_file(0) {
        for (unsigned i = 0; i < BufferCount; i++) {
            char szTmp[128];
            sprintf(szTmp, "%d", i);
            std::string name = "qrf_top_Qi_M_real_memcore_U_";
            name += szTmp;

            buffer[i] = new qrf_top_Qi_M_real_memcore(name.c_str());
            buffer[i]->clk      ( clk );
        ...
        if (dump_vcd) {
            m_trace_file = sc_create_vcd_trace_file( "qrf_top_Qi_M_real.trace");
            // ports
